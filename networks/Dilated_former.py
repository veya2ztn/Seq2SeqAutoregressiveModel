...
from networks.utils.Attention import Dilated_attn
...
class Block(nn.Module):
    def __init__(self, dim, window_size, num_heads=1, mlp_ratio=4., 
                ...
                act_layer=nn.ReLU, norm_layer=nn.LayerNorm, dilated_size=[1,1,1]):
        super().__init__()
        self.dim = dim
        ...
        self.attn = Dilated_attn(
            dim, window_size=self.window_size, num_heads=num_heads, qkv_bias=qkv_bias,
            attn_drop=attn_drop, proj_drop=drop,  dilated_size=dilated_size)
    ...
    def forward(self, x, attn_mask):
        ...
        # W-MSA/SW-MSA
        x = self.attn(x, mask=attn_mask)  # [B, T, H, W, C]
...
class Layer(nn.Module):
    def __init__(self, dim, depth, window_size, 
                ...
                drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, dilated_reverse=False) -> None:
        super().__init__()
        self.dim = dim
        self.depth = depth
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                dilated_size=[2**((i+1)//2),2**((i+1)//2)] if i % 2==1 else [1,1]
            )
    ...
    def __init__(self, patch_size=(1, 1, 1), in_chans=3, out_chans=20,
        ...
        for i_layer in range(self.num_layers):
            ...
            layers = Layer(dim=embed_dim,
                                ...
                                norm_layer=norm_layer,
                                dilated_reverse=False if i_layer < 2 else True
                                )
