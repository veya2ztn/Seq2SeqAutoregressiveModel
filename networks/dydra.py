...
import numpy as np
from networks.utils.utils import drop_path_f, DropPath, window_partition, window_reverse, PatchEmbed
import copy
from networks.utils.utils import Mlp
from functools import partial
from networks.utils.Attention import Flash_attn, Hydra_attn
...
class ScaleOffset(nn.Module):
    def __init__(self, dim, scale=True, offset=True) -> None:
        ...
        if scale:
            ...
            nn.init.trunc_normal_(self.gamma, std=.02)
        else:
            self.gamma = None
        if offset:
...
class HydraBlock(nn.Module):
    def __init__(self, dim, window_size, num_heads=1, mlp_ratio=4., 
                ...
                act_layer=nn.ReLU, norm_layer=nn.LayerNorm, local=True, use_attn=True):
        super().__init__()
        self.dim = dim
        ...
        self.GAU1 = Hydra_attn(
            dim, window_size=self.window_size, num_heads=num_heads, qkv_bias=qkv_bias,
            attn_drop=attn_drop, proj_drop=drop, expansion_factor=1, local = local, use_attn=use_attn)
    ...
    def forward(self, x, attn_mask):
        ...
        # W-MSA/SW-MSA
        x = self.GAU1(x, mask=attn_mask)  # [B, T, H, W, C]
...
class HydraLayer(nn.Module):
    def __init__(self, dim, depth, window_size, 
                ...
                drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, use_attn=True) -> None:
        super().__init__()
        self.dim = dim
        self.depth = depth
...
class Hydrafromer(nn.Module):
    ...
    def __init__(self, patch_size=(1, 1, 1), in_chans=3, out_chans=20,
        ...
        for i_layer in range(self.num_layers):
            ...
            layers = HydraLayer(dim=embed_dim,
                                ...
                                norm_layer=norm_layer,
                                use_attn=True if i_layer < 2 else True
                                )
...
class Hydra(nn.Module):
    def __init__(self, patch_size=(1,1,1), in_chans=20, out_chans=20, embed_dim=96, window_size=[4,8], depths=[2, 2, 6, 2], \
                num_heads=[3, 6, 12, 24], Weather_T=16) -> None:
        super().__init__()
        self.net = Hydrafromer(patch_size=patch_size, in_chans=in_chans, out_chans=out_chans, \
