...
class Block(nn.Module):
    def __init__(self, dim, window_size, num_heads=1, mlp_ratio=4., 
                ...
                act_layer=nn.GELU, norm_layer=nn.LayerNorm, depth=0, swin=False):
        super().__init__()
        self.dim = dim
...
class SWD_former(nn.Module):
    def __init__(self, img_size=[32,64], patch_size=(1,1,1), in_chans=20, out_chans=20, embed_dim=768, window_size=[4,8], depths=[2, 2, 6, 2], \
        ...
        super().__init__()
        self.net = SWDfromer(img_size=img_size, patch_size=patch_size, in_chans=in_chans, out_chans=out_chans, \
