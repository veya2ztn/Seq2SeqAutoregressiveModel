...
from networks.utils.Blocks import Windowattn_parallelblock
from einops import rearrange
from fairscale.nn.checkpoint.checkpoint_activations import checkpoint_wrapper
from megatron_utils.tensor_parallel.layers import RowParallelLinear, Parallel_PatchEmbed
...
class Layer(nn.Module):
    def __init__(self, dim, depth, window_size, 
                ...
                use_checkpoint=False, use_cpu_initialization=True) -> None:
        super().__init__()
        self.dim = dim
        self.depth = depth
        ...
        for i in range(depth):
            if layer_type == "window_block":
                block = Windowattn_parallelblock(
                        dim=dim,
...
class LG_net(nn.Module):
    ...
    def __init__(self, img_size=[32, 64], patch_size=(1, 1, 1), in_chans=3, out_chans=20,
                 ...
                 use_checkpoint=False, use_cpu_initialization=True):
        super().__init__()
        ...
        self.patch_embed = Parallel_PatchEmbed(img_size=img_size,
        ...
        self.final = RowParallelLinear(embed_dim, out_chans*patch_size[-1]*patch_size[-2], 
...
class LGNet_parallel(nn.Module):
    def __init__(self, img_size=[32, 64], patch_size=(1,1,1), in_chans=20, out_chans=20, 
                 embed_dim=768, window_size=[4,8], depths=[2, 2, 6, 2], \
                 num_heads=[3, 6, 12, 24], Weather_T=16, drop_rate=0., 
                 attn_drop_rate=0., drop_path=0., use_checkpoint=False,
                 use_cpu_initialization=True) -> None:
        super().__init__()
