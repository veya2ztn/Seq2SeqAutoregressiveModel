...
from networks.utils.Attention import SD_attn, WindowAttention, HiLo, Conv_attn, SD_attn_parallel, SD_attn_Cross
...
class Windowattn_Cross_block(nn.Module):
    def __init__(self, dim, window_size, num_heads=1, mlp_ratio=4.,
                 qkv_bias=True, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 attn_type="windowattn", pre_norm=True, **kwargs):
    ...
    def forward(self, x, cross):
        ...
        if self.pre_norm:
            x = shortcut + self.drop_path(self.attn(self.norm(x), (self.norm(cross))))
        else:
            x = self.norm(shortcut + self.drop_path(self.attn(x, cross)))
