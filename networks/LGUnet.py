...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=[32, 64], patch_size=[2,2], in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
...
class Enc_net(nn.Module):
    def __init__(self, img_size=[32, 64], patch_size=[2,2], inchans=20, embed_dim=96, lgnet_embed=768, depths=[2,2,2],
        ...
        self.inchans_list = []
        if inchans % 37 != 0:
            self.inchans_list.append(inchans % 37)
        for i in range(inchans//37):
            self.inchans_list.append(37)
        
        self.enc_list = nn.ModuleList()
        for i in range(len(self.inchans_list)):
            self.enc_list.append(Transformer_Encoder(img_size=img_size, patch_size=patch_size, in_chans=self.inchans_list[i],
                                                     ...
                                                     use_checkpoint=use_checkpoint))
        self.proj = nn.Linear(embed_dim*len(self.inchans_list)*2**(len(depths)-1), lgnet_embed)
    ...
    def forward(self, x):
        ...
        outdata_list = []
        for i in range(len(self.inchans_list)):
            data, data_downsample_list = self.enc_list[i](data_split[i])
...
class Dec_net(nn.Module):
    def __init__(self, img_size=[32, 64], patch_size=[2,2], outchans=20, embed_dim=96, lgnet_embed=768, depths=[2,2,2],
                 num_heads=[3,6,12], window_size=[6, 12], mlp_ratio=4, qkv_bias=True, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                 patch_norm=False,use_checkpoint=False):
        super().__init__()
        ...
        self.outchans_list = []
        if outchans % 37 != 0:
            self.outchans_list.append(outchans % 37)
        for i in range(outchans//37):
            self.outchans_list.append(37)
        ...
        for i in range(len(self.outchans_list)):
            ...
            self.final_proj_list.append(nn.Linear(embed_dim, self.outchans_list[i]*patch_size[0]*patch_size[1]))
    ...
    def forward(self, x, downsample_list):
        ...
        out_data_list = []
        for i in range(len(self.outchans_list)):
            out_data = self.dec_list[i](data_split[i], downsample_list[i])
...
class LGUnet(nn.Module):
    def __init__(self, img_size=[32, 64], patch_size=(1,1,1), in_chans=20, out_chans=20, embed_dim=768, window_size=[4,8], depths=[2, 2, 6, 2], \
        ...
        super().__init__()
        self.enc = Enc_net(img_size=img_size, patch_size=[2,2], inchans=in_chans, embed_dim=96, lgnet_embed=embed_dim, depths=[2,2,2],
                            num_heads=[3,6,12], window_size=window_size, mlp_ratio=4, qkv_bias=True, drop_rate=drop_rate,
        ...
        self.dec = Dec_net(img_size=img_size, patch_size=[2, 2], outchans=out_chans, embed_dim=96, lgnet_embed=embed_dim, depths=[2,2,2],
                            num_heads=[3,6,12], window_size=window_size, mlp_ratio=4, qkv_bias=True, drop_rate=drop_rate,
                            attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path, use_checkpoint=use_checkpoint) 
