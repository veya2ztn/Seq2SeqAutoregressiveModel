    ...
    def __init__(self, dim, window_size, num_heads=1, mlp_ratio=4., 
        ...
        if swin == False:
            if depth % 2 == 0:
                ...
                dilated_size = [2**((depth+3)//4), 2**((depth+3)//4), 2**((depth+3)//4)]
            ...
            if depth % 4 == 0:
                ...
                self.shift_size = [dilated_size[i]*window_size[i]//2 for i in range(len(window_size))]
            else:
                self.shift_size = [0 for _ in window_size]
        else:
            dilated_size = [1, 1, 1]
...
class TdownSample(nn.Module):
    def __init__(self, T, dim, drop=0., norm_layer=nn.LayerNorm) -> None:
        super().__init__()

        self.norm = norm_layer(T*dim)
        self.fc1 = nn.Linear(T*dim, T*dim)
        self.act = nn.GELU()
        self.drop1 = nn.Dropout(drop)
        self.fc2 = nn.Linear(T*dim, dim)
        self.drop2 = nn.Dropout(drop)
    
    def forward(self, x):
        B, T, H, W, C = x.shape
        x = self.norm(x.permute(0, 2, 3, 1, 4).reshape(B, H, W, -1))
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop1(x)
...
class SWDfromer(nn.Module):
    ...
    def __init__(self, patch_size=(1, 1, 1), in_chans=3, out_chans=20,
                 ...
                 norm_layer=partial(nn.LayerNorm, eps=1e-6), patch_norm=False, only_swin=False, Weather_T=6):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            ...
            layers = Layer(dim=embed_dim,
                                ...
                                window_size=window_size if i_layer < 2 else window_size[-2:],
            ...
            if i_layer == 1:
                down_layer = TdownSample(Weather_T, embed_dim)
        ...
        self.time_emb = nn.init.trunc_normal_(nn.Parameter(torch.zeros(Weather_T, embed_dim)), 0., 0.02)
    ...
    def forward(self, x):
        ...
        C = x.shape[-1]
        x = x.reshape(B, T, -1, C) + self.pos_embed
        x = x + self.time_emb.unsqueeze(-2)
        x = self.pos_drop(x).reshape(B, -1, C)
...
class SW3D_down(nn.Module):
    def __init__(self, patch_size=(1,1,1), in_chans=20, out_chans=20, embed_dim=768, window_size=[4,8], depths=[2, 2, 6, 2], \
        ...
        self.net = SWDfromer(patch_size=patch_size, in_chans=in_chans, out_chans=out_chans, \
                                        ...
                                        window_size=window_size, only_swin=only_swin, Weather_T=Weather_T)
