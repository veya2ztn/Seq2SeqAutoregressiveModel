...
class AFNONet(nn.Module):
    def __init__(self, img_size=None, patch_size=8, in_chans=20, out_chans=20, embed_dim=768, depth=12, fno_blocks=4, fno_bias=False,
        ...
        ##### normal ####
            ...
            # self.pre_logits = nn.Sequential(OrderedDict([
            #     ('conv1', nn.ConvTranspose2d(embed_dim, out_chans * 16, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))),
            #     ('act1', nn.Tanh()),
            #     ('conv2', nn.ConvTranspose2d(out_chans * 16, out_chans*4, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))),
            #     ('act2', nn.Tanh())
            # ]))
            # self.head = nn.ConvTranspose2d(out_chans * 4, out_chans, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        ##### less conv ####
        # elif patch_size == 1:
        #     self.pre_logits = nn.Sequential(OrderedDict([
        #         ('conv1', nn.Conv2d(embed_dim, out_chans * 8, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),
        #                             padding_mode='reflect')),
        #         ('act1', nn.Tanh()),
        #     ]))
        #     self.head = nn.Conv2d(out_chans * 8, out_chans, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),
        #                           padding_mode='reflect')
        #########################################################################
        #########################################################################
        #########################################################################
