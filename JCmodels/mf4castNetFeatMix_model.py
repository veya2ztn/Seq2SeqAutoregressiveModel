import torch
import sys
sys.path.append('/mnt/cache/gongjunchao/workdir/tmp/IfGAN_test/')
...
class AFNONet_MF_FeatMix(nn.Module): #model.patch_embed.proj.bias.dtype
    def __init__(self, clip_size=None, patch_size_T=1, patch_size=8, in_chans=20, out_chans=20, embed_dim=768, depth=12, mlp_ratio=4.,
        ...
        self.mix_layers_idx = kwargs.get('mix_layers_idx', [])
        ...
        self.mix_mhsas = nn.ModuleList([nn.MultiheadAttention(embed_dim=embed_dim, num_heads=4, dropout=0.1, batch_first=True) for i in range(len(self.mix_layers_idx))])
        self.mix_mlps = nn.ModuleList([nn.Linear(embed_dim * 2, embed_dim) for i in range(len(self.mix_layers_idx))])
    ...
    def _feat_mix(self, x, low_feats):
        """
        mix features in different layers.
        args:
            x: B, t*h*w, D
            low_feats: list[]. Element's shape is (B, t*h*w, D). 
        """
        mix_feats = []
        out = torch.zeros_like(x, device=x.device)
        for i, feat in enumerate(low_feats):
            feat, _ = self.mix_mhsas[i](x, feat, feat, need_weights=False)
            mix_feat_coeff = torch.sigmoid(self.mix_mlps[i](torch.cat([x, feat], dim = -1)))
            # import pdb; pdb.set_trace()
            mix_feats.append(mix_feat_coeff * feat)
        for i, feat in enumerate(mix_feats):
            out = out + feat
        out = out / np.sqrt(len(mix_feats))
        return out
    ...
    def forward_features(self, x):
        ...
        if not get_args().checkpoint_activations:
            for i, blk in enumerate(self.blocks):
                x = blk(x) # (B, t*h*w, D)
                if i in self.mix_layers_idx:
                    self.low_feats.append(x)
        else:
            x = checkpoint_sequential(self.blocks, 4, x)

        x = self._feat_mix(x, self.low_feats)        
...
if __name__ == "__main__":
    ...
    out_chans = 20
    B = 3
    clip_pixels = clip_size[0] * clip_size[1] * clip_size[2]
    x = torch.arange(0, B*clip_pixels*in_chans).reshape((B, clip_size[0], in_chans, clip_size[1], clip_size[2])).float()
    mix_layers_idx = [3, 6, 9, 11]

    global_afno_mf = AFNONet_MF_FeatMix(clip_size=clip_size, patch_size_T=2, patch_size=2, in_chans=in_chans, out_chans=out_chans, embed_dim=128, depth=12, mlp_ratio=4.,
                 uniform_drop=False, drop_rate=0., drop_path_rate=0., norm_layer=None, dropcls=0, mask_area=0., mix_layers_idx=mix_layers_idx)
    y = global_afno_mf(x)

# srun -p ai4science -c 96  --gres=gpu:0 python debug.py  
