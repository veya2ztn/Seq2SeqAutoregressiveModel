...
class Time_Sphere_Model(BaseModel):
    ...
    def __init__(self, args, backbone):
        ...
        self.timespace_shape=(args.history_length,*img_size) if args.history_length>1 else img_size
    def forward(self, x, x_timestamp, y_timestamp):
        ...
        _input = torch.stack([x*torch.cos(year_pos_x),
        ...
        extra_loss= 0
        if self.block_target_timestamp:
            year_pos_y = y_timestamp[...,0]
            day_pos_y = y_timestamp[...,1]
            output_direction = torch.Tensor([torch.cos(year_pos_y),
                            torch.sin(year_pos_y)*torch.cos(day_pos_y),
                            torch.cos(year_pos_y)*torch.sin(day_pos_y)])
            output_direction_shape=[B,3]+[1]*len(self.timespace_shape)        
            output_direction = output_direction.reshape(*output_direction_shape)
            extra_loss = torch.nn.functional.cosine_similarity(x,n,dim=1).mean()
        x = x.norm(dim=1)
        return x, extra_loss
