...
class GraphCast(MeshCast):
    ...
    - Mesh to Grid
    -------------------------------------
    Need the external mesh file generate by Mathmatica.
    -------------------------------------
    the input is a tensor (B, P, W, H), but the whole internal tensors are shape (B, L ,P)
    where the L equal the node number or edge number.
    -------------------------------------
    We always use the `im2col` way to compute the Aggeragating of Graph.
    - it is efficient for Edge2Node since each edge only used once
    - it is ineffcient for Node2Edge that each node will be used multi-times (This is where Package like DGL can optimize) 
    '''
...
class Node2Edge2NodeBlock(nn.Module):
    """
    Main Concern for using 
    `self.STE2E_S2E = nn.Parameter(STE2E_S2E)` 
    rather than 
    `self.STE2E_S2E = nn.Linear(in, out)`
    is for the usage for dgl module where will use matrix multiple directly on node. 
...
class GraphCastFast(MeshCast):
    ...
    def __init__(self, config):
        super().__init__()
        self.img_size  = img_size  = config.img_size
        self.in_chans  = in_chans  = config.in_chans
        self.out_chans = out_chans = config.out_chans
        self.embed_dim = embed_dim = config.embed_dim
        self.depth     = depth     = config.depth
        
        self.graphflag = graphflag = config.graphcast.graphflag
        self.agg_way   = agg_way   = config.graphcast.agg_way
        self.nonlinear = nonlinear = config.graphcast.nonlinear
    ...
    def forward(self, _input, return_feature = False):
        B, P , W, H =_input.shape
        feature_along_latlot     = self.grid_rect_embedding_layer(_input.permute(0,2,3,1).flatten(1,2))
        grid_rect_embedding      = feature_along_latlot[:,self.G2M_grid2LaLotudePos]
        grid_rect_embedding      = torch.cat([self.northsouthembbed.repeat(B,1,1),grid_rect_embedding],1)
        ...
        grid_mesh_bond_embedding,grid_rect_embedding,mesh_node_embedding = self.layer_grid2mesh(grid_mesh_bond_embedding,grid_rect_embedding,mesh_node_embedding)
        for mesh2mesh in self.layer_mesh2mesh:
        ...
        grid_rect_embedding      = grid_rect_embedding[:,self.M2G_LaLotudePos2grid] #(B,64,128,embed_dim)
        out = self.projection(grid_rect_embedding).permute(0,3,1,2)
        if return_feature:
            out.append(return_feature.permute(0,3,1,2))
