...
import pandas as pd
import copy
from utils.tools import get_sub_luna_point,get_sub_sun_point, dprint
...
def get_default_timestamp_pool(config):
    ...
    return {'train':np.arange(np.datetime64("1979-01-02"), np.datetime64("2017-01-01"), np.timedelta64(time_unit, "h")),
            ...
            'ftest': np.arange(np.datetime64("2018-01-01"), np.datetime64("2019-01-01"), np.timedelta64(time_unit, "h")),
            }
...
def get_timestamp_date(split, config):
    ...
    if timestamps is None:
        ...
        dprint(f"you don't assign a timestamp list for {split}, then we use the default timestamp config  per {time_unit} hour")
    else:
        dprint(f"noice you assign your own year timestamps list from {timestamps.min} to {timestamps.max()}" )
...
def preload_full_dataset(split, config):
    ...
    if os.path.exists(fast_offline_data_name):
        dprint(f"detect fast offline data in {fast_offline_data_name}, use this")
        ...
        dprint(f"dont find offline data in {fast_offline_data_name}")
        dprint("then seek split file store in format {root}/{year}/{year}-{hour:04d}.npy")
        loaded_tensor  = None
        ...
        dprint("finish preload the whole dataset into memory")
...
class FakeWeatherBench(WeatherBenchBase):
    def __init__(self, split="train", shared_dataset_tensor_buffer=(None, None), with_idx=False, config={}, debug=False):
        ...
        self.time_intervel = config.time_intervel
        self.resolution_h, self.resolution_w = self.img_size = config.img_size
...
class WeatherBench(WeatherBenchBase):
    ...
    def __init__(self, split="train", shared_dataset_tensor_buffer=(None, None),with_idx=False, config={}):
        self.root = config.get('root', self.default_root)
        assert self.root is not None
        dprint(f"use dataset in {self.root} for {split}")
        self.split = split
        self.with_idx=with_idx
        self.timestamp_date=get_timestamp_date(split, config)
        self.single_data_path_list = get_init_file_list(self.timestamp_date)
        ...
        self.normlized_flag = config.normlized_flag 
        self.use_time_feature = config.use_time_feature 
        self.time_reverse_flag = config.time_reverse_flag 
        self.add_LunaSolarDirectly = config.add_LunaSolarDirectly 
        self.offline_data_is_already_normed = config.offline_data_is_already_normed 
        self.constant_channel_pick = config.constant_channel_pick 
        self.make_data_physical_reasonable_mode = config.make_data_physical_reasonable_mode 
        
        self.data_processing_is_all_done = False
        ...
        if self.add_LunaSolarDirectly:
            self.LaLotude, self.LaLotudeVector = self.get_mesh_lon_lat(self.resolution_h, self.resolution_w)  # (32, 64 )
...
class WeatherBenchPatch(WeatherBench):
    """
    config = {
        'root':"/mnt/data/ai4earth/zhangtianning/datasets/WeatherBench/weatherbench_6hour/",
        'channel_name_list':"configs/datasets/WeatherBench/2D70.channel_list.json",
    ...
    """
    def __init__(self, split="train", shared_dataset_tensor_buffer=(None,None),config = {}, debug=False):
        super().__init__(split=split, shared_dataset_tensor_buffer=(None, None), config=config)
        self.patch_range  = patch_range = config.data_patch_range
        self.cross_sample = ((self.split == 'train') or debug) and (self.patch_range is not None)
        self.patch_chunk  = config.patch_chunk
        ...
        dprint(f"notice we will use around_index{self.around_index.shape} to patch data")
        #### due to the data efficiency, reshape the odata at beginning and do data preprocess
        assert self.dataset_tensor is not None
        assert (self.record_load_tensor == 1).all()
        self.dataset_tensor =  self.dataset_tensor[:,self.channel_choice] #(B, 70, 32, 64) or (B, 5, 14, 32, 64)
        self.dataset_tensor = torch.Tensor(self.dataset_tensor)
        if not self.offline_data_is_already_normed:
            self.dataset_tensor = self.normlizer.do_pre_normlize(self.dataset_tensor)
        self.dataset_tensor = self.dataset_tensor.float()
        self.data_processing_is_all_done = True
        self.img_shape  = self.dataset_tensor.shape[2:]
        self.length = len(self)

    def get_cross_sample_patch(self, idx):
        '''
        Notice for 3D case, we return (5,14,32,64) data tensor
        '''
        # if self.use_offline_data:
        ...
        if len(self.img_shape)==2:
            center_h = np.random.randint(self.img_shape[-2] - (self.patch_range[-2]//2)*2,size=(self.patch_chunk,)) 
            center_w = np.random.randint(self.img_shape[-1],size=(self.patch_chunk,))
            location = self.around_index[center_h, center_w] #(B,2,5,5) 
            patch_idx_h = location[:,0]#(B,5,5)
            patch_idx_w = location[:,1]#(B,5,5)
            batch_idx = np.random.randint(self.length,size=(self.patch_chunk,)).reshape(self.patch_chunk,1,1) #(B,1,1)
            data = []
            for i in range(self.time_step):
                dt = i*self.time_intervel
                output = {'field':self.dataset_tensor[batch_idx+dt,:,patch_idx_h,patch_idx_w].permute(0,3,1,2)}
                if self.use_time_feature:
                    output['timestamp'] = [self.timestamp_date[idx+dt] for idx in batch_idx]
                if self.add_LunaSolarDirectly:
                    output['sun_mask']  = np.stack([self.get_sum_mask(idx)[0][a,b] for idx,a,b in zip(batch_idx,patch_idx_h,patch_idx_w)])
                    output['luna_mask'] = np.concatenate([self.get_moon_mask(idx)[0][a,b] for idx,a,b in zip(batch_idx,patch_idx_h,patch_idx_w)])
                assert not (self.constant_channel_pick and len(self.constant_channel_pick) > 0), 'do not use constant'
                output['location']  = location
                data.append(output)
            #data = [[self.data[batch_idx+i,:,patch_idx_h,patch_idx_w].permute(0,3,1,2)] for i in range(self.time_step)]
        elif len(self.img_shape)==3:
            center_z    = np.random.randint(self.img_shape[-3] - (self.patch_range[-3]//2)*2,size=(self.patch_chunk,)) 
            center_h    = np.random.randint(self.img_shape[-2] - (self.patch_range[-2]//2)*2,size=(self.patch_chunk,))  
            center_w    = np.random.randint(self.img_shape[-1],size=(self.patch_chunk,)) 
            ...
            batch_idx = np.random.randint(self.length,size=(self.patch_chunk,)).reshape(self.patch_chunk,1,1,1) #(B,1,1,1)
            data = []
            for i in range(self.time_step):
                output = {'field': self.dataset_tensor[batch_idx+dt, :,patch_idx_z, patch_idx_h, patch_idx_w].permute(0, 4, 1, 2, 3)}
                if self.use_time_feature:
                    output['timestamp'] = [self.timestamp_date[idx+dt] for idx in batch_idx]
                if self.add_LunaSolarDirectly:
                    output['sun_mask']  = np.stack([self.get_sum_mask(idx)[0][a,b] for idx,a,b in zip(batch_idx[...,0],patch_idx_h,patch_idx_w)])
                    output['luna_mask'] = np.concatenate([self.get_moon_mask(idx)[0][a,b] for idx,a,b in zip(batch_idx[...,0],patch_idx_h,patch_idx_w)])
                assert not (self.constant_channel_pick and len(self.constant_channel_pick) > 0), 'do not use constant'
                output['location']  = location
                data.append(output)
            #data = [[self.data[batch_idx+i,:,patch_idx_z,patch_idx_h,patch_idx_w].permute(0,4,1,2,3)] for i in range(self.time_step)]
        else:
            raise NotImplementedError
        return data
    ...
    def __getitem__(self, idx):
        if self.cross_sample:
            return self.get_cross_sample_patch(idx)
        else:  
            return self.get_normal_image(idx)

    @staticmethod
    def collate_fn(batch):
        if isinstance(batch[0], dict):
            return {key: WeatherBenchPatch.collate_fn([d[key] for d in batch]) for key in batch[0]}
        elif isinstance(batch[0], list):
            return [WeatherBenchPatch.collate_fn(samples) for samples in zip(*batch)]
...
if __name__ == "__main__":
    ...
    #         dprint(str(i) + ' processor started !')
    #     p.close()
    #     p.join()
    ...
    # dprint(ERA5CephDataset.__name__)
    # dprint(dataset[0])
    
    # dprint(len(dataset[0]))
    # dprint(len(dataset[1]))
