#!/bin/sh
#SBATCH -J AFA      # 作业在调度系统中的作业名为myFirstJob;
#SBATCH -o log/%j-AFA.out  # 脚本执行的输出将被保存在20210827-%j.out文件下，%j表示作业号;
#SBATCH -e log/%j-AFA.out  # 脚本执行的输出将被保存在20210827-%j.out文件下，%j表示作业号;
...
#python optuna_run.py --train_set 2D706N --dataset_type WeathBench7066 --epoch 100 --mode pretrain --dataset_flag 3D70U --save_warm_up 1000 --wrapper_model ConVectionModel --debug 0 --use_amp 0
python optuna_run.py --train_set 2D706N --dataset_type WeathBench7066 --epoch 100 --mode pretrain --save_warm_up 1000
